...
> Baseline de código auditado: commit `a74986d93098367b9e0b2085ed6d113f29ffa1d0`, por inspeção estática; build e hardware não executados.
...
| EVD-012 | Uma análise estática histórica registrou estimadores ausentes e pipeline principal limitado a RAW. | observação de código histórica | SRC-PRJ-003, problemas 1.1-1.3 | E2 | superada para o HEAD atual; preservada para rastrear a evolução |
| EVD-013 | A mesma análise histórica apontou produção a 100 sps, consumo unitário mais lento e ausência de controle da FIFO. | observação de código histórica | SRC-PRJ-003, problemas 1.4-1.7 | E2 | o HEAD atual drena a FIFO em lote e detecta overflow, mas ainda requer TST-001/TST-002 |
...
| EVD-018 | No commit auditado, o controlador executa qualidade, HR, SpO₂ e confiança; o driver lê IDs, verifica configuração e drena FIFO em lote. A curva de SpO₂ permanece não calibrada e a telemetria ativa é incompleta para uso clínico. | inspeção estática versionada | SRC-PRJ-005; `main/app`, `drivers`, `processing`, `safety`, `transport`, `storage` | E2 | demonstra presença de código, não compilação, comportamento em hardware ou exatidão |
| EVD-019 | Não foram localizados fontes, diretórios ou relatórios de testes automatizados/HIL para o firmware no commit auditado. | ausência de artefato versionado | SRC-PRJ-005; árvore do repositório | E2 | testes externos não anexados continuam não verificáveis |
...
- Estimativa de SpO₂ habilitada com `calibrated=false` para a prova de conceito.
- Telemetria ativa sem `clinical_valid`, calibração, motivos de invalidade, overflow e status dos estimadores.
- Ausência de testes automatizados e HIL versionados no baseline auditado.
